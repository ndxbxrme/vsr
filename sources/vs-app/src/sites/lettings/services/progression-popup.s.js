...
(function() {
  'use strict';
  angular.module('vs-lettings-inner').factory('LettingsProgressionPopup', function($timeout, $http, lettingsProperty, Auth, alert) {
    ...
    return {
      ...
      setCompleted: function() {
        if (data) {
          hidden = true;
          $http.post($http.sites["lettings"].url + '/api/milestone/completed', {
            milestone: data._id,
            roleId: lettingsProperty.get().RoleId
      ...
      setProgressing: function() {
        if (data) {
          hidden = true;
          $http.post($http.sites["lettings"].url + '/api/milestone/start', {
            milestone: data._id,
            roleId: lettingsProperty.get().RoleId
          }, $http.sites.lettings.config);
      ...
      setDate: function(date) {
        if (data) {
          ...
          return lettingsProperty.get().$case.save();
        }
      },
      ...
      addNote: function(note) {
        if (data && note) {
          ...
          lettingsProperty.get().$case.save();
          return note = '';
        }
      },
      getNotes: function() {
